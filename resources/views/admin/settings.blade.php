...
<form action="{{ route('admin.settings.update') }}" method="POST" class="space-y-6">
    ...
    <section class="grid gap-6 xl:grid-cols-3">
        ...
        <div class="rounded-[32px] border border-white/60 bg-white/72 p-6 shadow-[0_22px_60px_rgba(47,47,47,0.07)] backdrop-blur-xl">
            <p class="text-xs font-semibold uppercase tracking-[0.26em] text-[var(--color-olive)]">Prompt Management</p>
            <h2 class="mt-3 font-[var(--font-display)] text-3xl leading-tight">Global prompt controls</h2>

            <div class="mt-6 space-y-5">
                <label class="block">
                    <span class="mb-2 block text-sm font-semibold text-[var(--color-charcoal)]">Global Prompt Prefix</span>
                    <textarea name="global_prompt_prefix" rows="4" class="w-full rounded-[18px] border border-[rgba(47,47,47,0.10)] bg-[rgba(247,246,242,0.84)] px-4 py-3 text-sm outline-none" placeholder="Example: Generate an elegant interior concept with warm lighting and balanced composition.">{{ $config['global_prompt_prefix'] }}</textarea>
                    <span class="mt-2 block text-xs text-[color:rgba(47,47,47,0.52)]">Prepended to every AI prompt before the style-specific prompt.</span>
                </label>

                <label class="block">
                    <span class="mb-2 block text-sm font-semibold text-[var(--color-charcoal)]">Global Prompt Suffix</span>
                    <textarea name="global_prompt_suffix" rows="4" class="w-full rounded-[18px] border border-[rgba(47,47,47,0.10)] bg-[rgba(247,246,242,0.84)] px-4 py-3 text-sm outline-none" placeholder="Example: Keep the result realistic, premium, and visually cohesive.">{{ $config['global_prompt_suffix'] }}</textarea>
                    <span class="mt-2 block text-xs text-[color:rgba(47,47,47,0.52)]">Appended to every AI prompt to standardize output quality.</span>
                </label>
            </div>
        </div>
    ...
    <section class="grid gap-6 xl:grid-cols-3">
        ...
        @foreach ($budgetCards as $budget)
            <div class="rounded-[32px] border border-white/60 bg-white/72 p-6 shadow-[0_22px_60px_rgba(47,47,47,0.07)] backdrop-blur-xl">
                <p class="text-xs font-semibold uppercase tracking-[0.26em] text-[var(--color-olive)]">Budget Configuration</p>
                <h2 class="mt-3 font-[var(--font-display)] text-3xl leading-tight">{{ ucfirst($budget['key']) }} budget</h2>

                <div class="mt-6 space-y-5">
                    <label class="block">
                        <span class="mb-2 block text-sm font-semibold text-[var(--color-charcoal)]">Label</span>
                        <input type="text" name="budget_{{ $budget['key'] }}_label" value="{{ $budget['label'] }}" class="w-full rounded-[18px] border border-[rgba(47,47,47,0.10)] bg-[rgba(247,246,242,0.84)] px-4 py-3 text-sm outline-none">
                    </label>

                    <div class="grid gap-4 md:grid-cols-2">
                        <label class="block">
                            <span class="mb-2 block text-sm font-semibold text-[var(--color-charcoal)]">Min Price</span>
                            <input type="number" min="0" name="budget_{{ $budget['key'] }}_min" value="{{ $budget['min'] }}" class="w-full rounded-[18px] border border-[rgba(47,47,47,0.10)] bg-[rgba(247,246,242,0.84)] px-4 py-3 text-sm outline-none">
                        </label>

                        <label class="block">
                            <span class="mb-2 block text-sm font-semibold text-[var(--color-charcoal)]">Max Price</span>
                            <input type="number" min="0" name="budget_{{ $budget['key'] }}_max" value="{{ $budget['max'] }}" class="w-full rounded-[18px] border border-[rgba(47,47,47,0.10)] bg-[rgba(247,246,242,0.84)] px-4 py-3 text-sm outline-none">
                        </label>
                    </div>

                    <label class="block">
                        <span class="mb-2 block text-sm font-semibold text-[var(--color-charcoal)]">Budget Prompt</span>
                        <textarea name="budget_{{ $budget['key'] }}_prompt" rows="5" class="w-full rounded-[18px] border border-[rgba(47,47,47,0.10)] bg-[rgba(247,246,242,0.84)] px-4 py-3 text-sm outline-none">{{ $budget['prompt'] }}</textarea>
                        <span class="mt-2 block text-xs text-[color:rgba(47,47,47,0.52)]">Used by AI generation whenever a user selects the {{ $budget['key'] }} budget tier.</span>
                    </label>
                </div>
            </div>
        @endforeach
